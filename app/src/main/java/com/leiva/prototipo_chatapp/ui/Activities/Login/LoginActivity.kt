...
class LoginActivity : AppCompatActivity() {
    ...
    private fun enviarOTP() {
        val telefono = L_et_telefono.text.trim().toString()

        if (telefono.isNotEmpty()) {
            // Verificar la longitud del número si el código de país es España
            if (selector_codigo_pais_L.selectedCountryNameCode == "ES" && telefono.length != 9) {
                Toast.makeText(this@LoginActivity, "El número de teléfono debe tener 9 dígitos", Toast.LENGTH_SHORT).show()
                return
            }
            ...
            if (isNetworkAvailable()) {
                ...
                // Muestra un mensaje de advertencia si no hay conexión a Internet
                Toast.makeText(applicationContext, "No hay conexión a Internet", Toast.LENGTH_SHORT).show()
            }
        } else {
            // Si el campo de teléfono está vacío, muestra un mensaje de Toast
            Toast.makeText(applicationContext, "Por favor, introduzca un teléfono", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun iniciarSesion() {
        ...
        val password = L_et_password.text.trim().toString()

        if (telefono.isNotEmpty() && otp.isNotEmpty()  &&password.isNotEmpty()) {
            if (isNetworkAvailable()) { // Verificar disponibilidad de red
                CoroutineScope(Dispatchers.Main).launch {
